<div class="min-h-screen bg-gray-100 py-6 flex flex-col justify-center sm:py-3">
	<x-slot name="header">Creacion licencia</x-slot>
      ...
      <div class="max-w-md mx-auto">
        <div class="flex items-center space-x-5">
          ...
          <div class="block pl-2 font-semibold text-xl self-start text-gray-700">
            <h2 class="leading-relaxed">Creacion licencia</h2>
        ...
        <div class="divide-y divide-gray-200">
          <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
            ...
            <div class="flex flex-col">
              ...
              <select wire:model="licencia.categoria_id">
              	<option>Seleccione</option>
	              @foreach($categorias as $categoria)
	              	<option value="{{$categoria->id}}">{{$categoria->categoria}}</option>
	              @endforeach
              </select>
              @error('licencia.categoria_id')<span>{{$message}}</span>@enderror
            </div>
            <div class="flex flex-col">
              <label class="leading-loose">Restricciones</label>
              <input wire:model="licencia.restricciones" type="text" class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600">
              @error('licencia.restricciones')<span>{{$message}}</span>@enderror
            </div>
            <div class="flex flex-col">
              <label class="leading-loose">Tipo sangre</label>
              <input wire:model="licencia.tipo_sangre" type="textArea" class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600">
              @error('licencia.tipo_sangre')<span>{{$message}}</span>@enderror
            </div>
            <div class="flex items-center space-x-4">
              <div class="flex flex-col">
                <label class="leading-loose">Fecha expedicion</label>
                <div class="relative focus-within:text-gray-600 text-gray-400">
                  <input wire:model="licencia.fecha_expedicion" type="date" class="pr-4 pl-10 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600">
                </div>
                @error('licencia.fecha_expedicion')<span>{{$message}}</span>@enderror
              </div>
              <div class="flex flex-col">
                <label class="leading-loose">Fecha vencimiento</label>
                <div class="relative focus-within:text-gray-600 text-gray-400">
                  <input wire:model="licencia.vigencia" type="date" class="pr-4 pl-10 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600">
                </div>
                @error('licencia.vigencia')<span>{{$message}}</span>@enderror
              </div>
            </div>
          </div>
