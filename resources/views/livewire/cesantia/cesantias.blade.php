<div class="flex flex-col items-center h-auto pb-6 pt-6">
    <x-slot name="header">Cesantias</x-slot>
    ...
    <div class="mb-4" x-data="{ open: true }" x-transition.duration.500ms>
        <div class="flex max-w-sm w-full bg-white shadow-md rounded-lg overflow-hidden mx-auto" x-show="open">
            ...
            <div class="w-full flex justify-between items-start px-2 py-2">
                ...
                <button href="" x-on:click="{ open = !open}">
            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{$cesantia->nombre}}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{$cesantia->cod_proteccion_social}}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{$cesantia->telefono}}</div>
                  </td>
                  <td class="flex flex-row gap-2 px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    {{-- Edit --}}
                    <button class="text-indigo-600 hover:text-indigo-900" wire:click="updateModal({{$cesantia->id}})">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </button>
                    {{-- show --}}
                    <button class="text-indigo-600 hover:text-indigo-900" wire:click="delete({{$cesantia->id}})">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
